import torch
import numpy as np
from RotateSoftNMS.rotateiou import intersections
...
def soft_nms_pytorch(dets, box_scores, sigma=0.5, thresh=0.5, cuda=0):
    ...
    y4 = dets[:, 7]
    scores = box_scores
    areas = np.sqrt((x1 - x3) ** 2 + (y1 - y3) ** 2) * np.sqrt((x1 - x4) ** 2 + (y1 - y4) ** 2)
    ...
    for i in range(N):
        ...
        # 计算当前最大框和其他剩余所有框的iou
        # ---------------------------------------------------
        ovr = np.array(intersections(dets[i,: -1], dets[pos:, : -1]))

        # ---------------------------------------------------------------------------------
        # 采取高斯的方式衰减置信度,这里其实会循环衰减，因为i在变，maxpos， maxscore取出来的都是下一次的最大
        # ---------------------------------------------------------------------------------
    ...
    # select the boxes and keep the corresponding indexes
    mask = scores < thresh
    no_keep = dets[:, -1][mask].astype(np.int)
